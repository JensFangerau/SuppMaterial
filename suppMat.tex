...
We are using a 2D model for two reasons. First, we focus on a comparison to the real cells located in the master cell file of the VLR. Cells in the master cell file contribute most to the growth of the VLR and feature the property that these are mostly located within a plane in 3D. This means that they can be projected onto a 2D plane given in side view without (too much) loss of information. Second, such a model is much easier to describe than in 3D which significantly reduces the model complexity and therefore the analysis. Our intention is the creation of an idealized model based on simple rules that allows a comparison to the real datasets.

\noindent
Our model requires the application of a underlying \textbf{model surface} that represents the developing VLR. Furthermore, the positioning and the number of \textbf{founder cells} that contribute to the development have to be determined. Additionally, the \textbf{division setting} has to be defined, i.e.~the type and time point of a division event. In a nutshell, the simulation starts with a predefined number and positioning of founder cells and develops over time based on the underlying surface structure. During the simulation, the VLR and the cells within grow and increase in size. According to the chosen division properties, a cell will divide if it exceeds a specific area threshold or area ratio with respect to its initial size right after its birth. Through this, we generate a convenient model representing the growth of the VLR.

In the next sections, we will give a description of the used underlying model surface and the settings of the initial founder cells and divisions. Afterwards, we will explain the different model scenarios addressed in the main text.

\subsection{Model Surface}
\noindent
The choice of the underlying surface for the model fundamentally affects the development of the tissue. This means that a change of vertex positions on the surface influences the evolving cells of the model, their walls and junctions. In our case, we use \textit{B\'ezier surfaces} to create an idealized model that allows an investigation of a common growth behaviour of the VLR.
...
\caption[]
{
{\bf First and last B\'ezier surfaces of degree $(6, 6)$ for the idealized model.} The mapping of $(u_i, v_i)$ to the changing position on the surface is unique over time.
}
...
\caption[]
{
{\bf Division configurations for the model.}
}
...
\noindent
The last thing missing is the setting of the division properties in the model. This means that the time point and axis orientation of the division that splits a cell into two daughter cells have to be determined. We use non-deterministic division rules for each model, i.e.\ a random variation of orientation of the division axis. By repeated simulations (e.g.\ 100) of such a probabilistic model with the same setting we can analyse its division behaviour based on statistically solid results.
...
\noindent
Using the B\'ezier surfaces, the setting of founder cells as well as the division properties, we are able to generate a VLR model. We research how the division behaviour is influenced when cells are dividing based on a specific rule or randomly. Furthermore, we investigate the robustness of the VLR model when influencing certain properties of the model. In total, we consider four different model scenarios:

\noindent
\textbf{Geometric}\\
\noindent
Here, we choose the first setting in having two founder cells with equal sizes and shapes. The division axes are chosen based on the Besson-Dumais division rule (see Figure 5D-F in main text).

\noindent
\textbf{Random}\\
\noindent
The initial cell configuration is identical to the geometric one but the division axes are now chosen randomly (see Figure 5G-I in main text).

\noindent
\textbf{1\textsuperscript{st} random then geometric}\\
\noindent
The initial cell configuration is again identical to the geometric one but we use a random rule for the first division round and then use the Besson-Dumais division rule for all later divisions (see Figure 5J-L in main text). By this, we can analyse the robustness of the VLR model.

\noindent
\textbf{Basal growth}\\
\noindent
We check if the VLR growth could be responsible for the emergence of the high-order pattern of periclinal divisions. For this purpose, we change the deformation behaviour of the B\'ezier surface from an equidistant distribution of control points to a distribution in which the control points are growing faster towards the dome tip (see Figure~\ref{fig:basalGrowth}). We force here the first division round to be an asymmetric anticlinal one and we use the Besson-Dumais rule. As a result, the model leads to more dominant cell divisions at the base of the VLR. This model impairs the high-order pattern of periclinal divisions (see Figure 5M-O in main text) although the layer formation and division orientations are similar to the model results using an equidistant distribution of control points. \TODO{@Alexis: in the main text we are using 1DC for the basal growth; should we maybe only use NH? Then all models share the same cell founder setting.}
%
\begin{figure}[htbp]
	\begin{center}
...
\subsubsection{Visualisation of Model}
\noindent
...
\noindent
We run several simulations of one model using division rules with randomized variation, each resulting in one division sequence tree as explained in Section~\ref{sec:treeDivisionSequence}. However, we want to visually check how often and where the high-order pattern of periclinal divisions is preserved for all simulations. This allows us to use statistically solid model results to compare them with the real datasets.
%
\begin{figure}[htbp]
	\begin{center}
