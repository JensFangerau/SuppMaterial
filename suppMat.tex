...
\author[1,2,5]{Daniel von Wangenheim}
...
\noindent
For Model 6, the orientation of the division axes are chosen randomly for all divisions but we apply our approach of preserving the cell shape as explained in Section~\ref{sec:cellShape} (see Figure~\ref{fig:visumodel}).
...
\begin{figure}[htbp]
\centering
	\begin{overpic}[width=1.0\linewidth]{images/Model_divisions.pdf}
	\end{overpic}
\caption[Averaged division sequence trees for the model results.]
{{\bf Averaged division sequence trees for the model results.} For each model scenario (see Section~\ref{sec:modelScenarios}) with 100 simulations, an averaged division sequence tree is shown (see Section~\ref{sec:visualAnalysis}) that illustrates the percentage in which the upper layers were generated before the lower ones.
}
\label{fig:Modeldivisions}
\end{figure}
%
...
\begin{Itemize}
  ...
  \end{Itemize}
  \TODO{@Alexis: please update all model movies and their links.}
  \item \emph{Geometric} model of lateral root formation, corresponding to Figure 5D (main text) \href{https://youtu.be/GLUKCE4nN3A}{\textcolor{blue}{[LINK]}}.
  \item \emph{Random} model of lateral root formation, corresponding to Figure 5G (main text) \href{https://youtu.be/V9xkyPfRQeM}{\textcolor{blue}{[LINK]}}.
  ...
  \item \emph{1\textsuperscript{st} asymmetric then geometric} model of lateral root formation, corresponding to Figure~\ref{fig:visumodel} \href{https://youtu.be/ypBdyjFyqYw}{\textcolor{blue}{[LINK]}}.
  \item \emph{Random with equal areas} model of lateral root formation, corresponding to Figure~\ref{fig:visumodel}
  \TODO{add movie link}
